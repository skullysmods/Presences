...
function fetchNextData<T>(): T | null {
...
presence.on('UpdateData', async () => {
  ...
  if (privacyMode) {
    presenceData.details = 'Watching ITVX'
  }
  else {
    switch (path) {
      case '/': {
      ...
      case '/watch': {
        ...
        const currentChannelMetadata = nextData.props.pageProps.channels.find(
        ...
        const epNum = currentSlot.episodeNumber
